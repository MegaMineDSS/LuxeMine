...
bool Admin::checkLoginCredentials(const QString &username, const QString &password)
{
    ...
    db.close();
    if (!valid) QMessageBox::warning(this, "Login Failed", "Invalid username or password.");
    return valid;
}
...
{
    if (column == 2) { // Last PDF Path column
